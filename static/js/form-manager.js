...
class FormManager {
  ...
  initializeAsyncFields() {
    ...
    const aulasAssincronasSim = document.querySelector(
      'input[name="aulas_assincronas"][value="sim"]'
    );
  ...
  setupConditionalFields() {
    ...
    const aulasAssincronasRadios = document.querySelectorAll(
  ...
  updateExistingUnits(modalidade) {
    ...
    unidades.forEach((unidade, index) => {
      ...
      const enderecoInputs = unidade.querySelectorAll(
        'input[name="endereco_unidade[]"], input[name="bairro_unidade[]"]'
      );
      ...
      if (modalidade === "Online") {
        ...
        enderecoLabels.forEach((label) => {
          if (
            label.textContent.includes("Endereço") ||
            label.textContent.includes("Bairro")
          ) {
  ...
  togglePlataformaDigital() {
    ...
    const modalidade = modalidadeSelect.value;
    const plataformaContainer = document.getElementById(
      "plataforma_digital_container"
    );
    const plataformaDigitalField =
      document.getElementById("plataforma_digital");
  ...
  toggleAulasAssincronas(isAsync) {
    ...
    const horariosContainer = document.getElementById(
      "horarios_detalhados_online_container"
    );
    const horariosOnlineContainer = document.getElementById(
      "horarios_online_container"
    );
    ...
    const horariosDetalhados = document.querySelectorAll(
      ".horarios-detalhados-container"
    );
    const horariosOnline = document.querySelectorAll(
      ".horarios-online-container"
    );
  ...
  generateUnidadeHTML(count, isOnline) {
    const legendText = isOnline
      ? `Informações do Curso ${count}`
      : `Informações da Unidade ou Turma ${count}`;
    const unidadeIndex = count - 1;
    ...
    return `
            <fieldset class="unidade-fieldset">
                ...
                <div class="checkbox-group dias-aula">
                    ${this.generateDaysCheckboxes(unidadeIndex)}
                </div>
            </fieldset>
        `;
  }
      ...
      .map(
        (day) => `
            <label class="checkbox-label">
                <input type="checkbox" name="dias_aula_presencial_${unidadeIndex}[]" value="${day}">
                <span>${day}</span>
            </label>
        `
      )
  ...
  renumberUnits() {
    ...
    unidades.forEach((unidade, index) => {
      ...
      if (legend) {
        const legendText = isOnline
          ? `Informações do Curso ${index + 1}`
          : `Informações da Unidade ou Turma ${index + 1}`;
  ...
  renumberPlataformas() {
    ...
    plataformas.forEach((plataforma, index) => {
      ...
      if (legend) {
        legend.innerHTML = `Informações da Plataforma ${
          index + 1
  ...
  updateRemovePlataformaButtonsVisibility() {
    const plataformas = document.querySelectorAll(
      "#plataforma_list .plataforma-item"
    );
